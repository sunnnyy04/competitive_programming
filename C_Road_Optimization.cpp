...
int solve2(int i,int j,int n,vvi &dp,vi &d,vi &a,int l){
    if(i==n){
        return 0;
    }
    ...
    for(int k=i+1;k<=n;k++){
        if(k-i-1>j) break;
        if(k==n){
            ans=min(ans,(a[i]*(l-d[i])+solve2(k,j-(k-i-1),n,dp,d,a,l)));
        }
        else ans=min(ans,(a[i]*(d[k]-d[i])+solve2(k,j-(k-i-1),n,dp,d,a,l)));
    }
    return dp[i][j]=ans;
...
void solve() {
    ...
    loop(i,0,n) cin>>a[i];
    vvi dp(n+1,vi(k+1,-1));
    cout<<solve2(0,k,n,dp,d,a,l)<<endl;
}
