...
void solve() {
    ...
    for (ll i = 0; i < n; i++) {
        if (a[i] == 2) p[i]++;
        else if (a[i] == 3) q.insert(i);
        if (i > 0) p[i] += p[i - 1];
    }

    ll buffer = 0;
    for (auto j : q) {
        buffer = modadd(buffer, expo(2, p[j], MOD998), MOD998);
    }

    ll ans = 0;
    for (int i = 0; i < n; i++) {
        if (a[i] == 1) {
            while (!q.empty() && i > *q.begin()) {
                buffer = modsub(buffer, expo(2, p[*q.begin()], MOD998), MOD998);
                q.erase(q.begin());
            }
            if (q.empty()) continue;

            ll denom_inv = expo(expo(2, p[i], MOD998), MOD998 - 2, MOD998);
            ll term = modmul(buffer, denom_inv, MOD998);
            term = modsub(term, (ll)q.size(), MOD998);
            ans = modadd(ans, term, MOD998);
        }
    }
