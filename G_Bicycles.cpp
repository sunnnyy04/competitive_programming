...
void solve() {
    ...
    vector<map<int, int64_t>> dis(n + 1);

    priority_queue<tuple<int64_t, int64_t, int64_t>, vector<tuple<int64_t, int64_t, int64_t>>, greater<>> pq;
    dis[1][speed[1]] = 0;
    pq.push({0, 1, speed[1]});

    while (!pq.empty()) {
        auto [d, node, v] = pq.top();
        pq.pop();


        for (auto [to, w] : adj[node]) {
            if (!dis[to].count(v) || dis[to][v] > d + (w * v)) {
                dis[to][v] = d + (w * v);
                pq.push({dis[to][v], to, v});
            }

            if (v > speed[node]) {
                if (!dis[to].count(speed[node]) || dis[to][speed[node]] > d + (w * speed[node])) {
                    dis[to][speed[node]] = d + (w * speed[node]);
                    pq.push({dis[to][speed[node]], to, speed[node]});
                }
            }
        }
    }
