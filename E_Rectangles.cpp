...
void solve() {
    ...
    auto transpose = [&](const vector<string> &mat, int n, int m) {
    ...
    auto compute = [&](vector<string> &mat, int n, int m) -> vvi {
        vvi ans(n, vi(m, INT_MAX));
        for (int i = 0; i < n; i++) {
            vector<int> z(m, INT_MAX);
            for (int j = i + 1; j < n; j++) {
                int k = 0;
                while (k < m) {
                    int f = -1;
                    while (k < m && (mat[i][k] != '1' || mat[j][k] != '1')) k++;
                    f = k;
                    k++;
                    while (k < m && (mat[i][k] != '1' || mat[j][k] != '1')) k++;
                    if (k >= m) break;
                    for (int l = f; l <= k; l++) {
                        int area = (j - i + 1) * (k - f + 1);
                        z[l] = min(z[l], area);
                        ans[i][l] = min(ans[i][l], area);
                        ans[j][l] = min(ans[j][l], area);
                    }
                }
            }
            for (int l = 0; l < m; l++)
                for (int j = n - 2; j >= i; j--)
                    ans[j][l] = min(ans[j][l], ans[j + 1][l]);
        }
        return ans;
    };

    auto print = [&](const vvi &ans) {
        for (auto &row : ans) {
            for (auto &val : row)
    ...
    if (n <= m) {
        auto ans = compute(a, n, m);
        print(ans);
    } else {
        auto aT = transpose(a, n, m);
        auto ans = compute(aT, m, n);
        vvi ansT(n, vi(m));
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++)
                ansT[j][i] = ans[i][j];
