...
void solve() {
    ...
    auto mark = [&](int i, int j) {
        ...
        if (i > 0 && a[i - 1][j] == val) hasSameAdj = true;
        if (i < n - 1 && a[i + 1][j] == val) hasSameAdj = true;
        if (j > 0 && a[i][j - 1] == val) hasSameAdj = true;
        if (j < m - 1 && a[i][j + 1] == val) hasSameAdj = true;
    ...
    for (auto &[val, score] : mpp) {
        ans += score;
        if (score == 2) hasTwo = true;
    }

    cout << (hasTwo ? ans - 2 : ans - 1) << nl;
}
