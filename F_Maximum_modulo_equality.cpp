...
void solve() {
    ...
    vector<int>v;
    for(int i=1;i<n-1;i++){
        if (abs(a[i] - a[i-1]) != abs(a[i] - a[i+1]) && max(abs(a[i] - a[i-1]),abs(a[i] - a[i+1]))%min(abs(a[i] - a[i-1]),abs(a[i] - a[i+1]))!=0) {
            v.push_back(i);
        }
    }
    sort(v.begin(),v.end());
    ...
    while(q--){
        ...
        int x=upper_bound(all(v),l-1)-v.begin();
        int y=lower_bound(all(v),r-1)-v.begin();
        if(x<y){
            cout<<1<<" ";
            continue;
        }
        else{
            if (abs(a[l] - a[l-1]) != abs(a[l] - a[l+1])){
